...
void AWidgetManager::InitializeWidgets()
{
	...
	if (FPSWidgetClass && !FPSWidget)
	{
		...
		if (FPSWidget)
		{
			UE_LOG(LogTemp, Log, TEXT("WidgetManager: FPS Widget created (not added to viewport yet)"));
            
			// Load setting và apply visibility
			if (UGameInstance* GameInstance = GetWorld()->GetGameInstance())
			{
				if (USettingsSubsystem* SettingsSubsystem = GameInstance->GetSubsystem<USettingsSubsystem>())
				{
					FS_GraphicsSettings GraphicsSettings = SettingsSubsystem->GetAllSettings().GraphicsSettings;
                    
					if (GraphicsSettings.bFPSEnable)
					{
						FPSWidget->AddToViewport(0);
						FPSWidget->SetVisibility(ESlateVisibility::Visible);
						UE_LOG(LogTemp, Log, TEXT("WidgetManager: FPS Widget shown (from saved settings)"));
					}
					else
					{
