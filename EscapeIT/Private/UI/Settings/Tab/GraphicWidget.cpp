...
void UGraphicWidget::NativeDestruct()
{
    ...
    if (FPSRow)
        FieldOfViewRow->OnSelectionChanged.RemoveDynamic(this, &UGraphicWidget::OnShowFPSChanged);
...
void UGraphicWidget::InitializeSelectionRows()
{
    ...
    const TArray<FText> FPSOptions = MakeToggleOptions();
    ...
    if (FPSRow)
    {
        FPSRow->InitializeRow(FPSOptions, static_cast<int32>(CurrentSettings.bFPSEnable ? 1 : 0), FText::FromString(TEXT("Show FPS")));
        FPSRow->OnSelectionChanged.AddDynamic(this, &UGraphicWidget::OnShowFPSChanged);
    }
}
...
TArray<FString> UGraphicWidget::ValidateSettings() const
{
    ...
    if (CurrentSettings.bFPSEnable)
    {
        UE_LOG(LogTemp, Warning, TEXT("GraphicWidget: Show FPS must be initialied"));
    }
...
void UGraphicWidget::OnShowFPSChanged(int32 NewIndex)
{
    if (bIsLoadingSettings) return;
    CurrentSettings.bFPSEnable = (NewIndex == 1);
    MarkAsCustomPreset();
    UE_LOG(LogTemp, Log, TEXT("GraphicWidget: ShowFPS changed to %s"), 
           CurrentSettings.bFPSEnable ? TEXT("On") : TEXT("Off"));
    UpdateFPSWidgetVisibility(CurrentSettings.bFPSEnable);
    if (SettingsSubsystem) SettingsSubsystem->ApplyGraphicsSettings(CurrentSettings);
}

void UGraphicWidget::SetWidgetManager(AWidgetManager* InWidgetManager)
{
